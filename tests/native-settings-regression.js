(async () => {
  ...
  const saved = p.settings[key],
    otherBefore = p.settings[other];
  ...
  const hiddenMatches = app.setting.searchIndex
    .search("assets")
    .filter((r) => r.tab.id === "quiet-tree");
  ...
  for (
    let i = 0;
    i < 4 &&
    JSON.parse(await app.vault.adapter.read(".obsidian/plugins/quiet-tree/data.json"))[key] !== 620;
    i++
  )
  ...
  if (
    p.settings[key] !== 620 ||
    p.settings[other] !== otherBefore ||
    JSON.stringify(p.settings.excluded) !== exclusions
  )
    throw Error("Slider persistence changed the wrong settings");
  await p.saveSettings({ [key]: saved });
  app.setting.open();
  app.setting.openTabById("quiet-tree");
  return {
    mobile,
    ...
    sliderPersisted: true,
    otherDelayPreserved: true,
