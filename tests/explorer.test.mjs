...
test('C / D boundary has distinct A and root destinations', () => {
  ...
  assert.equal(
    describeTarget(tree, choices[1], 'b').label,
    '根目录 · A 与 D 之间',
  );
});
...
test('first, last, empty and collapsed destinations are usable', () => {
  ...
  const collapsed = moveNode(
    last,
    'gesture',
    insideTarget(last, 'archive', 0),
  ).tree;
      ...
      for (let gap = 0; gap <= rows.length; gap++)
        for (const target of gapTargets(tree, rows, gap, id)) {
          ...
          for (const node of Object.values(next.nodes))
            assert.equal(
              childrenOf(next, node.parentId).filter((x) => x === node.id)
                .length,
              1,
            );
        }
...
test('B below C retains both outcomes with no additional canvas height', () => {
  ...
  assert.deepEqual(
    moveNode(tree, 'b', hit.candidates[0]).tree.nodes.a.children,
    ['c', 'b'],
  );
  assert.deepEqual(
    moveNode(tree, 'b', hit.candidates[1]).tree.roots.slice(0, 3),
    ['a', 'b', 'd'],
  );
...
test('picker does not select an option when it appears under an unmoved pointer', () => {
  ...
  assert.equal(
    pickerHit(p, p.left + 50, p.top + p.header + 70, 3, 0),
    'bridge',
  );
...
test('desktop chooser has a reachable bridge, and leaving it clears the hit', () => {
  ...
  assert.equal(
    pickerHit(p, (p.originX + p.left) / 2, p.originY + 18, 2, 0),
    'bridge',
  );
  assert.equal(
    pickerHit({ ...p, armed: true }, p.left + 30, p.top + 30 + 66, 2, 0),
    1,
  );
});
