...
export const scenarios = [
  ...
  },
  {
    ...
    detail: '嵌套、空目录与长文件名',
    instruction:
      '试试把文件移入空目录，或从第三层拖回根目录。拖动文件夹时，其内容会一起移动。',
  },
  {
    id: 'scroll' as const,
    name: '长列表',
    detail: '滚动、悬停展开与首尾插入',
    instruction:
      '拖动时靠近列表上下边缘可自动滚动。停留在折叠目录中部，会自动展开目录。',
  },
];
...
export function validateMove(
  ...
  if (
    ...
    tree.nodes[target.parentId]?.kind !== 'folder'
  )
    return '目标不是文件夹';
  if (isInSubtree(tree, target.parentId, id))
    return '不能放入自身或自己的子目录';
  if (
    target.beforeId !== null &&
    !childrenOf(tree, target.parentId).includes(target.beforeId)
  )
    return '插入位置已变化';
  return null;
}
export function moveNode(
  ...
  destination.splice(
    target.beforeId === null
      ? destination.length
      : destination.indexOf(target.beforeId),
    0,
    id,
  );
  ...
  const changed =
    ...
    childrenOf(tree, sourceParent).join('\0') !==
      childrenOf(next, sourceParent).join('\0');
...
export function isNoopMove(
  ...
  return (
    target.beforeId === id ||
    target.beforeId === (siblings[siblings.indexOf(id) + 1] ?? null)
  );
}
...
export function describeTarget(
  ...
  const ids = childrenOf(tree, target.parentId).filter(
    (id) => id !== draggedId,
  );
