...
export class ExplorerSettingsTab extends PluginSettingTab {
  ...
  private section(name: TextKey, help?: TextKey): HTMLElement {
    ...
    header.createEl('h3', { text: this.plugin.t(name) });
    if (help) {
  ...
  display() {
    ...
    for (const path of p.settings.excluded) {
      ...
      remove.addEventListener('click', async () => {
        ...
        await this.exclusions(
          p.settings.excluded.filter((item) => item !== path),
        );
    ...
    const commit = async () => {
      ...
      try {
        ...
        if (paths.length)
          await this.exclusions([
            ...new Set([...p.settings.excluded, ...paths]),
          ]);
      } catch (error) {
        p.report(error);
      } finally {
    ...
    attachment.addEventListener('click', async () => {
      ...
      await this.exclusions([...new Set([...p.settings.excluded, ...paths])]);
    ...
    if (p.store.error)
      containerEl.createEl('p', { text: t('readError'), cls: 'qt-error' });
  }
}
