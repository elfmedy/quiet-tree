export const en = {
  ...
  error:
    'Quiet Tree: could not apply the change. Check the JSON file and path.',
  invalidPath:
    'Use a vault-relative path without ..; the sort file must end in .json.',
  reservedPath: 'This path is reserved for Obsidian or plugin settings.',
  invalidJson:
    'Invalid order JSON: use directory keys and arrays of unique direct child names.',
  unavailable:
...
const zh: Record<TextKey, string> = {
  ...
  settings: 'Quiet Tree',
  intro:
    '直接整理原生文件列表。长按提起，移动选择位置，松手放下；按 Esc 取消。',
  path: '排序数据文件',
  pathHelp:
  ...
  delay: '长按延迟',
  delayHelp:
    '触屏长按时间，单位毫秒。鼠标至少等待 180 毫秒，并移动 4 像素后提起。',
  exclusions: '排除的目录',
  exclusionsHelp:
    '输入相对于知识库的目录路径，或从列表中选择。\n该目录及子目录的内部条目不排序、不记录；目录本身仍可在上一级调整位置。\n移除规则后恢复排序功能。',
  attachment: '加入当前附件目录',
  noAttachment:
    '当前没有独立的附件目录（不会自动排除根目录或相对当前笔记的目录）。',
  syntax: 'JSON 语法简介',
  syntaxHelp:
  ...
  rollbackFailed:
    '排序保存失败，且文件未能移回原位置。请检查文件当前位置后再继续。',
