...
test('destination copy separates the directory from the exact sibling position', () => {
  ...
  assert.equal(
    destinationLabel(tree, { ...inner, beforeId: 'c' }, 'b', (key) =>
      translate('zh', key),
    ).position,
    'C 之前',
  );
...
test('reject malformed documents and duplicate or non-direct children', () => {
  ...
  assert.deepEqual(parseOrder('{"__proto__": ["constructor"]}').__proto__, [
    'constructor',
  ]);
...
test('sort path is portable and cannot overwrite application settings', () => {
  ...
  assert.equal(
    orderPath('Meta/order.json', '.obsidian', 'quiet-tree'),
    'Meta/order.json',
  );
  for (const path of [
    '../order.json',
...
test('renames preserve positions and rewrite recorded descendants only', () => {
  ...
  assert.deepEqual(renameOrder(original, 'A/B.md', 'A/Z.md').A, [
    'Z.md',
    'C.md',
  ]);
